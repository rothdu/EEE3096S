...
// TODO: Debounce stuff (added to make this section easier to find)
uint32_t button0_prev_tick = 0; // initiliase "previous button0 tick" to 0
...
int main(void)
{
  ...
  char str[5]; // create string to store value
  sprintf(str, "%d", adc_val); // convert int to string

  writeLCD(str); // write string to LCD
...
void EXTI0_1_IRQHandler(void)
{
  ...
  // check if button press is reasonably long after previous one
  if (current_tick - button0_prev_tick > debounce_delay)
  {
    if (delay_t == 500) // change from 500 to 250
    {
      delay_t = 250;
    }
    else if (delay_t == 250) // change from 250 to 500
    {
      delay_t = 500;
    }

    button0_prev_tick = current_tick; // set current tick to previous
  }
...
void writeLCD(char *char_in){
  ...
  lcd_putstring(char_in); // write string to ADC
...
uint32_t pollADC(void){
  ...
  HAL_ADC_PollForConversion(&hadc, HAL_MAX_DELAY); // wait for ADC to finish converting
